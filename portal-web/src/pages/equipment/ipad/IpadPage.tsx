...
import { useIpads, useSaveIpads } from '@/modules/equipment/hooks/useEquipment'
...
export function IpadPage() {
  ...
  const handleSendRepair = (values: SendToRepairFormValues) => {
    ...
    message.success(`Sent ${selectedRowKeys.length} iPads to ${values.vendor}.`)
    setSelectedRowKeys([])
    setBulkStatus(null)
    setRepairOpen(false)
  }
  ...
  const handleChangeStatus = () => {
    ...
    message.success(t('equipment.ipad.statusUpdated', { status: statusLabel(bulkStatus), count: selectedRowKeys.length }))
    setSelectedRowKeys([])
    setBulkStatus(null)
  }
  ...
  return (
    <ListPageLayout
      title={t('equipment.ipad.listTitle')}
      description={t('equipment.ipad.listDesc')}
      actions={<Button icon={<Download className="h-4 w-4" />}>{t('equipment.ipad.exportList')}</Button>}
      ...
      filterBar={
        <>
          <Input
            ...
            placeholder={t('equipment.ipad.searchPlaceholder')}
      ...
      footer={
        <>
          <Text variant="caption" tone="secondary" className="tnum font-semibold">
            {filtered.length} of {devices.length} iPads
      ...
      <Table
        ...
        onRow={(record) => ({
          onClick: (event) => {
            ...
            navigate(`/equipment/ipad/${record.code}`)
          },
